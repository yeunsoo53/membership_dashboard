"""Add enum values to position title and level. Delete is_coordinator
...
def upgrade() -> None:
    ...
    op.execute("CREATE TYPE positiontitle AS ENUM ('MEMBER', 'COORDINATOR', 'PROJECT_DEVELOPER', 'CHAIR', 'VP', 'PRESIDENT', 'EXEC_SEC')")
    op.execute("CREATE TYPE positionlevel AS ENUM ('GC', 'EC', 'EB')")

    # ### commands auto generated by Alembic - please adjust! ###
    op.execute("ALTER TABLE position ALTER COLUMN title TYPE positiontitle USING title::positiontitle")
...
def downgrade() -> None:
    ...
    op.execute("ALTER TABLE position ALTER COLUMN title TYPE VARCHAR(255) USING title::text")
    
    op.execute("DROP TYPE positionlevel")
    op.execute("DROP TYPE positiontitle")
